-- ============================================================
-- FILE: impact_budget_evaluation_v3.sql
-- PURPOSE: Evaluate Impact Scores and Player Budgets for ALL
--          players across ALL countries and ALL formats.
--
-- KEY CHANGES FROM v2:
--   1. Normalization is now WITHIN ROLE GROUPS per format:
--      PARTITION BY format, role  (not just format)
--      Batters rank against batters, bowlers against bowlers.
--      Result: elite batters like Virat Kohli score correctly.
--
--   2. player_roles CTE fixed: both cricket_player_pool (SL/WI)
--      and cricapi_player_master (everyone else) are joined
--      independently. COALESCE(cpp.player_type, ipm.player_type)
--      picks up the right value from whichever table has it.
--      The old AND cpp.player_name IS NULL guard has been removed
--      because it prevented ipm from matching reliably.
--
-- RULES:
...
--
-- PREREQUISITE:
--   Run fix_player_roles.sql first to add player_type column
--   to cricapi_player_master and populate roles for all players.
--
-- RUN ORDER:
...
WITH
  ...
  player_roles AS (
    ...
    -- SL/WI roles come from cricket_player_pool (short country codes)
    LEFT JOIN cricket_player_pool cpp
     ...
     AND cpp.country = CASE p.country
           ...
           ELSE NULL END
    -- All other player roles come from cricapi_player_master
  ...
  FROM normalized
  ORDER BY format, role, country, player_name;
...
WITH
  ...
  SELECT
    ...
    final_impact_score,
    raw_budget_score,
    -- Budget: $50,000 (score=0) → $250,000 (score=100), rounded to nearest $5,000
...
-- In v3, every role group should show median ~50, since
-- normalization is within-group. Use this to confirm the
-- fix worked and no large 'unknown' group remains.
-- ============================================================

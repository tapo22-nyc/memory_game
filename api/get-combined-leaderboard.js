...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (fantasy_match_id) {
      ...
      // Match-wise: join Fantasy 11 scores with Closest Call scores for this fantasy match.
      // Uses FULL OUTER JOIN so users who played only one game still appear.
      rows = await sql`
        WITH cc_agg AS (
          SELECT
            p.user_id,
        ...
        SELECT
          ...
          1                                          AS matches_played
        FROM cc_agg cc
        FULL OUTER JOIN fantasy_user_match_scores f
          ON f.user_id = cc.user_id AND f.fantasy_match_id = ${matchId}

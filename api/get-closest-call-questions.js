...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const annotated = questions.map(q => {
      ...
      // Pre-match questions are only editable when match is 'open'.
      // Bonus questions (non-pre_match phase) can still be answered during a
      // locked match, but only within their own open/lock time window.
      const matchEditable = match.status === 'open' ||
                            (match.status === 'locked' && q.phase !== 'pre_match');
      const is_editable   = is_open_now && matchEditable;

      // Lock reason — open_time check must come before match-status check so
      // future bonus questions show their phase label rather than 'Match locked'.
      ...
      if (q.status === 'scored') {
        ...
        lock_reason = 'Match closed';
      } else if (match.status === 'locked' && q.phase === 'pre_match') {
        lock_reason = 'Predictions closed';
      }
      ...
      return {
        ...
        phase:                 q.phase,
        question_phase:        q.question_phase || 'pregame',
        ...
        actual_runs:           q.actual_runs   !== undefined ? q.actual_runs   : null,
        actual_wickets:        q.actual_wickets !== undefined ? q.actual_wickets : null,
        ...
        user_predicted_runs:     pred ? (pred.predicted_runs    !== undefined ? pred.predicted_runs    : null) : null,
        user_predicted_wickets:  pred ? (pred.predicted_wickets !== undefined ? pred.predicted_wickets : null) : null,
        user_predicted_choice:   pred ? (pred.predicted_choice  !== undefined ? pred.predicted_choice  : null) : null,
      };
    });

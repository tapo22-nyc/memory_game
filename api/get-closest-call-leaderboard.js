...
module.exports = async function handler(req, res) {
  ...
  const { closest_call_match_id } = req.query;
  ...
  try {
    ...
    if (closest_call_match_id) {
      ...
      // Overall leaderboard across all scored questions
      leaderboard = await sql`
        SELECT
          u.user_name,
          p.user_id,
    ...
    // Compute closest_calls: user had the lowest difference among all players for a question
    // We do this as a secondary query for efficiency
    let closestCallsMap = {};
    ...
    if (closest_call_match_id) {
      ...
      const closestRows = await sql`
        ...
        GROUP BY p.user_id
      `;
      for (const r of closestRows) {
        closestCallsMap[r.user_id] = Number(r.closest_calls);
      }
    } else {
      const closestRows = await sql`
        WITH min_diffs AS (

...
async function syncSingleMatch(cricapiMatchId) {
  const apiUrl =
  ...
  if (cricData.status !== 'success') {
    throw new Error(`CricAPI failed for match ${cricapiMatchId}`);
  }
  ...
  for (const innings of scorecards) {
    ...
    const bowlingTeam =
      cricData.data?.teams?.find(team => team !== battingTeam) || null;

    for (const bat of innings.batting || []) {
      const stats = getPlayer(bat.batsman, battingTeam);
      if (!stats) continue;
    ...
    for (const field of innings.catching || []) {
      const stats = getPlayer(field.catcher, bowlingTeam);
      if (!stats) continue;
...
export default async function handler(req, res) {
  try {
    const seriesUrl =
      `https://api.cricapi.com/v1/series_info?apikey=${process.env.CRICAPI_KEY}&id=${IPL_2026_SERIES_ID}`;
